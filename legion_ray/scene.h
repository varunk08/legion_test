...
struct HitInfo
{
  ...
  HitInfo() { Init(); }
  void Init() { z=BIGFLOAT; node=NULL; front=true; volume=false; shade=cyColor(0.0, 255.0, 0.0); renderIsoSurface=false;}
};
...
{
public:
	virtual cyColor	Illuminate(Point3 p) const=0;
...
public:
	...
	virtual cyColor Shade(const Ray &ray, const HitInfo &hInfo, const LightList &lights, int bounceCount) const=0;
